...
> 📖 **新项目直接看 [OPERATIONS_GUIDE.md](OPERATIONS_GUIDE.md)** — 11 条核心规范 + 新项目启动 6 步清单 + Claude 会自动遵守的规则清单。一页搞定上手。
...
```bash
bash ~/cocosSkill/init_project.sh <your-project-path>
```

自动完成：
- `lib/*.ts` → `<project>/assets/Script/infra/`
- `scripts/*.py` → `<project>/scripts/`
- `commands/*.md` → `<project>/.claude/commands/`
...
> cp ~/cocosSkill/CLAUDE.md your-project/CLAUDE.md  # 如没有
> ```
...
| **BaseUtil.ts** | 深拷贝、版本比较、灰度、震屏（已修 z 归零黑屏）、坐标转换 |
| **DateUtil.ts** | 日期工具：跨日（签到）、mm:ss 格式化、秒到下次刷新时间 |
| **ConfigLoader.ts** | 配置表加载：JSON 批量加载 + 主键索引 + 过滤 |

详细用法见 [lib/README.md](lib/README.md)。

## scripts/ Python 开发工具

Cocos 3.8 prefab/scene JSON 构造 + 常见 UI 模式快捷生成。**跑脚本前必须关闭 Cocos Creator**（自带 `ensure_cocos_closed()` 防呆）。

| 文件 | 用途 |
|---|---|
| **prefab_builder.py** | `PrefabBuilder` 类（prefab JSON 构造）+ UUID 压缩/解析 + scene patch |
...
```
cocosSkill/
├── install.sh                             # 一键全局安装（skills/memory/CLAUDE.md → ~/.claude/）
├── init_project.sh                        # 一键初始化新 Cocos 项目（lib+scripts+commands 到位）
├── CLAUDE.md                              # 全局开发铁律
├── README.md                              # 本文件
├── skills/                                # Skills（全局 ~/.claude/skills/）
│   ├── cocos-prefab-crud/                 # 最新：prefab/scene 增删改查纪律 + 踩坑合集 + T5 模板
...
├── lib/                                   # 通用代码库 TS（项目 assets/Script/infra/）
│   ├── EventBus.ts / SafePool.ts / ResLoader.ts / SoundManager.ts
│   ├── AnimationManager.ts / MathUtil.ts / Bezier.ts / HighlightManager.ts
│   ├── BaseUtil.ts（已修 shakeScreen z 归零 bug）
│   ├── DateUtil.ts（签到/倒计时 🆕）
│   └── ConfigLoader.ts（JSON 配置表 🆕）
├── scripts/                               # Python 开发工具（项目 scripts/）
│   ├── prefab_builder.py 🆕
│   └── ui_factories.py 🆕
└── memory/common/                         # 通用 memory（全局 ~/.claude/memory/）
```
